...
# SUDO — base sudo (nextbsd/nextbsd-userland#247). The binary must be setuid
# root (the cross install drops the bit; CI re-applies it after staging and
# after assemble-image's chown), it must run, and its policy plugin must load.
# /etc/sudoers, /etc/sudoers.d/{admin,wheel} and /etc/pam.d/sudo come from
# nextbsd-overlays; until /etc/sudoers is there, the policy checks SKIP.
