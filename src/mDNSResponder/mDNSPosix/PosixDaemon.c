...
int main(int argc, char **argv)
{
    ...
    // freebsd-launchd-mach: register the Mach service so a peer
    // bootstrap_look_up("com.apple.mDNSResponder") succeeds. Logs
    // MDNS-BOOT-OK on success; failure is non-fatal (daemon still
    // serves AF_UNIX uds clients).
    (void)mDNSResponderMachBridgeInit();
    ...
    // freebsd-launchd-mach: log the engine-init success marker for the boot test.
    if (mStatus_NoError == err)
        fprintf(stderr, "mDNSResponder MDNS-ENGINE-OK: mDNS_Init returned mStatus_NoError\n");
